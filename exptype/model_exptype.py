...
class ModelExptype(QSqlRelationalTableModel):

    def __init__(self):
        super(ModelExptype, self).__init__()
        logging.debug('%s.%s', __name__, inspect.currentframe().f_code.co_name)

        self.setTable("Exptype")
        self.setEditStrategy(QSqlTableModel.EditStrategy.OnManualSubmit)
        self.setJoinMode(QSqlRelationalTableModel.JoinMode.LeftJoin)

        self.idx_id = self.fieldIndex('ExptypeID')
        self.idx_name = self.fieldIndex('ExptypeName')

        self.setHeaderData(self.idx_id, Qt.Horizontal, "ID")
        self.setHeaderData(self.idx_name, Qt.Horizontal, "Exptype Name")
    ...
    def insert_row(self):
        ...
        record.setValue("ExptypeID", None)
        record.setValue("ExptypeName", "?name?")
