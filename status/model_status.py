...
class ModelStatus(QSqlRelationalTableModel):

    def __init__(self):
        super(ModelStatus, self).__init__()
        logging.debug('%s.%s', __name__, inspect.currentframe().f_code.co_name)

        self.setTable("Status")
        self.setEditStrategy(QSqlTableModel.EditStrategy.OnManualSubmit)
        self.setJoinMode(QSqlRelationalTableModel.JoinMode.LeftJoin)

        self.idx_id = self.fieldIndex('StatusID')
        self.idx_name = self.fieldIndex('StatusName')

        self.setHeaderData(self.idx_id, Qt.Horizontal, "ID")
        self.setHeaderData(self.idx_name, Qt.Horizontal, "Status Name")
    ...
    def insert_row(self):
        ...
        record.setValue("StatusID", None)
        record.setValue("StatusName", "?name?")
