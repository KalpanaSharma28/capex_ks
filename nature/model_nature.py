...
class ModelNature(QSqlRelationalTableModel):

    def __init__(self):
        super(ModelNature, self).__init__()
        logging.debug('%s.%s', __name__, inspect.currentframe().f_code.co_name)

        self.setTable("Nature")
        self.setEditStrategy(QSqlTableModel.EditStrategy.OnManualSubmit)
        self.setJoinMode(QSqlRelationalTableModel.JoinMode.LeftJoin)

        self.idx_id = self.fieldIndex('NatureID')
        self.idx_name = self.fieldIndex('NatureName')

        self.setHeaderData(self.idx_id, Qt.Horizontal, "ID")
        self.setHeaderData(self.idx_name, Qt.Horizontal, "Nature Name")
    ...
    def insert_row(self):
        ...
        record.setValue("NatureID", None)
        record.setValue("NatureName", "?name?")
